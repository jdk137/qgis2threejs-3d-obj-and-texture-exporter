    ...
    */
    ...
    project.layers.forEach(function (layer) {
      ...
      layer.build(app.scene);
        ...
        var result = exporter.parse( app.scene );
        console.log('exp');


        var content = result;
        var blob = new Blob([content], {type: "text/plain;charset=utf-8"});
        saveAs(blob, "out" + (+new Date()) + ".obj");//saveAs(blob,filename)

    /*
        var floatingDiv = document.createElement( 'textarea' );
        document.body.appendChild( floatingDiv );
        floatingDiv.style.display = 'block';
        floatingDiv.innerHTML =  result;
        //console.log(result);
    */
    ...
    //if (app.labels.length) {app.scene.add(app.labelConnectorGroup); console.log('t');}
...
Q3D.ModelBuilder.COLLADA = function (project, model) {
  ...
  if (model.src !== undefined) {
    ...
    var xmlParser = new DOMParser(),
        ...
        url = "./";
    console.log('data', JSON.stringify(this.loader.parse(responseXML, undefined, url)));
